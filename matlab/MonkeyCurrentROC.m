...
function [] = MonkeyCurrentROC(Tank,Block)
SaveData=0;
MyTank=Tank;
...
TT.SetEpocTimeFilterV('Levl',0,.05);
TT.SetFilterWithDescEx(x);
...
[~,y] = size(levl);
[~,y1] = size(trs);
if y > 270 % deletes extraneous trials
    y = 270;
    freq = freq(:,1:270);
    levl = levl(:,1:270);
    nlvl = nlvl(:,1:270);
...
LowestSpikes=round((min(N)/10)-1)*10;
LowestPlotSpikes=round((min(N)/10)-1)*10;
StepSizePlot=2;%ceil((max(N)-min(N))/10);
...
if SaveData==1;
    SaveThreshData(MyTank,MyBlock,NeuroThresh(1),BehaviorThresh(1),...
        ...
        BehavThreshFit(1),NeuroThreshFit(1),RSBstd)
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Plotting
%
...
try
    ...
    c=coeff(3);
    n=coeff(4);
    x=(min(XDataIn):(max(XDataIn)-min(XDataIn))/500:max(XDataIn));
    y=WeibulCurveFitFunction(x,a,b,c,n);  %createFit3 is thus far the best fit over createFit.
