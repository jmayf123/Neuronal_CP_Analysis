...
fopen('Thresh.txt','w');
fopen('Slope.txt','w');
CP = cell(1,length(dur));
Grand_CP = cell(1,length(dur)); 
p_values = cell(1,length(dur));

for dur_ct=1:length(dur)
    for i=1:loopcount
       [levl,N,CP{dur_ct},Grand_CP{dur_ct},p_values{dur_ct}] = MonkeyNandLevel(MonkeyName,MonkeyDataTank,TrialNum(i),RecordSite,SaveThresh,dur(dur_ct));
    end
end
    assignin('base','CP',CP) 
